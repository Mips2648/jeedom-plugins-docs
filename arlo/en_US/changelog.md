...
- Fix: since the last update, the siren equipment (integrated in some base) had by error the following commands: _Refresh_, _Connected_, _Mode_, _Schedule state_ which obviously did not work. These commands can be deleted on this device, they will no longer be created by the plugin.
