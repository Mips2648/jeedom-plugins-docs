...
- Se han añadido comandos para los equipos de tipo contenedor: **Estado de salud**, **En ejecución**, **En pausa**, **Reiniciando**, **Inactivo**, **OOMKilled** [(Ver documentación)]({{site.baseurl}}/{{page.pluginId}}/{{page.lang}})
...
- Se ha añadido compatibilidad con las instancias de Portainer de tipo **agente**
- Se han añadido comandos de información que indican el número de contenedores, volúmenes, imágenes, servicios y pilas en los equipos de tipo **agente** y **Docker**

## 2021-02-08

- Incorporación de un comando _URL pública_ en el equipo Docker
- Se ha añadido la visualización de la imagen utilizada para el contenedor en la página de configuración del equipo
- Se ha corregido un problema por el que los contenedores aparecían duplicados en el panel si se utilizaban varios Docker (es necesario forzar una sincronización manual para que la página de estado muestre la información correcta tras actualizar el complemento).
...
- Se ha corregido el problema por el que el comando «refrescar» no funcionaba en un dispositivo de tipo _Docker_.
- Se ha añadido el número de versión de Portainer en la página de estado
- Se ha añadido una opción en la configuración del complemento para elegir el modo de sincronización automática

## 2020-08-22

- Nueva página de estado que muestra una vista general de todos los contenedores y permite realizar todas las acciones básicas (similar a la página «Containers» de portainer.io)
- Un nuevo panel (que hay que activar en la configuración del complemento) recoge la misma información y las mismas acciones que la página de estado.
- Se ha añadido una configuración al complemento que permite definir el objeto principal por defecto que se utilizará al crear un nuevo contenedor, lo que te evitará tener que hacerlo manualmente cada vez.
...
- Al eliminar un dispositivo de Jeedom, los registros correspondientes a los contenedores también se eliminarán de tu Jeedom para facilitar el mantenimiento.
- Los dispositivos de tipo «Docker» tendrán un icono diferente al de los dispositivos de tipo «Container» para mejorar la visibilidad.
