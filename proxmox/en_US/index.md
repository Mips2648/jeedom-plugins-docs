...
- **Backup**: allows to take a backup. This command (of type message) has an 'email' field which can contain an email address to which a notification will be sent once the backup is finished (email sent by your Proxmox server) and an 'Options' field in which you can pass each desired option in the form _option=valeur_ (please use a space character to separate options; see the table below for a list of available options).
...
compress | Backup compression | the possible values are: `0|gzip|lzo|zstd` | `lzo`
